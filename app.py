...
# Clé API directe pour éviter les échecs de connexion
...
def moteur_expertise(idee, premium=False):
    ...
    for i, axe in enumerate(axes):
        try:
            # On demande explicitement du français
            ...
            # Extraction souple pour ne rien perdre
            textes = [res['content'] for res in r.get('results', []) if len(res['content']) > 40]
            if textes:
    ...
    st.markdown(f"""
        <div style="background: white; padding: 30px; border-radius: 15px; border: 1px solid #dee2e6; text-align: center;">
            ...
            <a href="https://buy.stripe.com/9" style="text-decoration:none;">
                <div style="background:#007bff;color:white;padding:15px;border-radius:10px;font-weight:bold;">ACCÉDER AU DOSSIER</div>
            </a>
        </div>
    """, unsafe_allow_html=True)
